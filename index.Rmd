...
#Find mean total crime per tract ??? DO I NEED THIS
...
crime_pop2010 <- left_join(pop2010_clean, crime2010, by = "NAME")
...
cpp17 <- cpp2017[ order(match(cpp2017$NAME, income_clean17$NAME)),]
cpp10 <- cpp2010[ order(match(cpp2010$NAME, income_clean10$NAME)),]

#Join income crime and income
income_cpp17 <- cbind(cpp17, income_clean17) %>%
  select(-NAME.1, -geometry.1)

income_cpp10 <- cbind(cpp10, income_clean10) %>%
  select(-NAME.1, -geometry.1)
``` 
...
#2010 regression
ggplot(income_cpp10) + geom_point(aes(x = MedianInc, y = meancpp)) +
  labs(x = "Median Income 2010", y = "Crime Per Capita 2010")

```
```{r}
ggplot(income_cpp17) + geom_point(aes(x = MedianInc, y = meancpp)) +
  labs(x = "Median Income 2017", y = "Crime Per Capita 2017")
```
...
mapinc10 <- ggplot(income_cpp10) + geom_sf(aes(fill = MedianInc)) +
  scale_fill_viridis_c() + labs(fill = "Median Income")

mapcrime10 <- ggplot(income_cpp10) + geom_sf(aes(fill = meancpp)) + 
...
mapinc17 <- ggplot(income_cpp17) + geom_sf(aes(fill = MedianInc)) +
  scale_fill_viridis_c(limits=c(10000,70000)) + labs(fill = "Median Income")

mapcrime17 <- ggplot(income_cpp17) + geom_sf(aes(fill = meancpp)) + 
  scale_fill_viridis_c() + labs(fill = "Average CPC")
