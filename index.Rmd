...
totalpop2017 <- get_acs(geography = "tract",
                        variables = c(medianinc = "B01003_001"),
                       state = "NY",
                       county = "Erie",
                       year = 2017,
                       geometry = TRUE)
...
colnames(pop2017_clean)[3] <- "Population"
colnames(pop2010_clean)[3] <- "Population"

###SOMETHING WRONG WITH MY JOIN HERE <<try left join again tomorrow
#inc_pop2017 <- st_join(select(pop2017_clean, Population), select(income_clean17, MedianInc))

#inc_pop2010 <- st_join(pop2010_clean, income_clean10, by = "NAME") %>%
...
crime_pop2017 <- left_join(inc_pop2017, crime2017, by = "NAME") %>%
  select(-parent_incident_type)

crime_pop2010 <- left_join(inc_pop2010, crime2010, by = "NAME") %>%
  select(-parent_incident_type)

cpp2017 <- crime_pop2017 %>%
  mutate(CrimePerCapita = (total/Population)) %>%
  group_by(NAME) %>%
  summarize(meancpp = mean(CrimePerCapita))

#NEED TO JOIN THIS WITH INCOME
...
income_cpp10 <- st_join(crime_pop2010, cpp2010, by = "NAME") %>%
  select(-total)
...
#2010 regression
ggplot(crimeAvg2010) + geom_point(aes(x = MedianInc, y = meancrime)) +
  labs(x = "Median Income 2010", y = "Mean Crime 2010")

ggplot(income_cpp17) + geom_point(aes(x = MedianInc, y = meancpp))

```

``` {r, results = 'asis', cache = TRUE}
#2017 regression
#regress2017 <- lm(Theft~MedianInc, data = crime_income2017)
#summary(regress2017)
...
This figure is a map of median income and median crime for 2010 by census tract in Buffalo, NY
```{r, fig.width=10, fig.height=5, fig.cap="2010 Income and Crime"}

mapinc10 <- ggplot(crime_income2010) + geom_sf(aes(fill = MedianInc)) +
  scale_fill_viridis_c() + labs(fill = "Median Income")

mapcrime10 <- ggplot(crimeAvg2010) + geom_sf(aes(fill = meancrime)) + 
  scale_fill_viridis_c() + labs(fill = "Average Crime")
...
This figure is a map of median income and median crime for 2017 by census tract in Buffalo, NY
```{r, fig.width=10, fig.height=5, fig.cap="2017 Income and Crime"}
mapinc17 <- ggplot(crime_income2017) + geom_sf(aes(fill = MedianInc)) +
  scale_fill_viridis_c(limits=c(10000,70000)) + labs(fill = "Median Income")

mapcrime17 <- ggplot(cpp2017) + geom_sf(aes(fill = meancpp)) + 
  scale_fill_viridis_c() + labs(fill = "Average Crime Per Capita")
